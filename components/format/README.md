...
- **Profile slices** (the hot, high-volume stream). The wire stores the derived nested **call-tree** (`{frameId, depth, start, duration}`), not raw samples — `capture` holds samples in memory and folds them to slices incrementally (`SliceBuilder`/`samplesToSlices` in [src/profile-slices.ts](src/profile-slices.ts)); only runs spanning **≥ ~1 sample interval** survive (`0.8×` nominal — jitter-tolerant, since CPU throttling drops the real cadence to ~8ms; single samples, duplicate-timestamp clusters, and Chrome's microsecond deep-recursion bursts drop to a `droppedSamples` count). The codec uses a bespoke **columnar** layout — four contiguous columns (frameId, depth as zigzag-delta, start-delta µs, duration in **1ms units** — inferred durations don't merit µs) — so nesting is implicit from depth + pre-order and the interned `stacks` table is gone entirely. On a 2000-slice fixture the whole capture packs to **<1 byte/slice** (v0's 6× capture: 2224 slices → 3.0 KB).
